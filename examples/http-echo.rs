...
use eventedco::net::tcp::{TcpStream, TcpListener};
...
fn main() {
    ...
    let listener = HttpListener::http(&bind_addr[..]).unwrap();

    let mut threads = Vec::new();
    for _ in 0..matches.value_of("THREADS").unwrap_or("1").parse().unwrap() {
        ...
        let th = thread::spawn(move|| {
            let co = Coroutine::spawn(move|| {
                loop {
                    ...
                    if keep_alive {
                        let c = Coroutine::spawn(move|| Worker(&echo_keepalive).handle_connection(&mut stream));
                        c.resume().unwrap();
                    } else {
                        let c = Coroutine::spawn(move|| Worker(&echo_close).handle_connection(&mut stream));
                        c.resume().unwrap();
                    }
                }
            });
